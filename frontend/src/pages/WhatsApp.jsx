      ...
      <div className="flex-1 flex flex-col">
        {currentChat ? (
          <>
            ...
            {/* Input de envio */}
            <ChatInput chatName={currentChat.name} onSent={fetchData} />
          </>
        ) : (
...
function ChatInput({ chatName, onSent }) {
  const [text, setText] = useState('')
  const [sending, setSending] = useState(false)

  const handleSend = async () => {
    if (!text.trim() || !chatName) return
    setSending(true)
    try {
      await axios.post('/api/whatsapp/send', { chatName, text: text.trim() })
      setText('')
      onSent?.()
    } catch (e) {
  ...
  return (
    <div className="p-3 border-t border-nexo-border flex items-center gap-2">
      <input
        type="text"
        ...
        disabled={sending}
        className="flex-1 px-4 py-2 bg-nexo-bg border border-nexo-border rounded-full text-sm text-nexo-text placeholder-nexo-muted focus:outline-none focus:border-nexo-success disabled:opacity-50"
      />
      <button
        onClick={handleSend}
        disabled={sending || !text.trim()}
        className="p-2 bg-nexo-success hover:bg-nexo-success/80 disabled:bg-nexo-muted text-white rounded-full transition-colors"
      >
        {sending ? (

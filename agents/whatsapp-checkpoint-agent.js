...
async function openGroup(page, groupName) {
  console.log(`[WhatsApp] Abrindo grupo: ${groupName}`);
  ...
  try {
    ...
    // Clica no grupo
    await page.locator(`text="${groupName}"`).first().click({ timeout: 10000 });
    await page.waitForTimeout(2000);
    console.log('[WhatsApp] Grupo aberto!');
    return true;
  } catch (e) {
    console.log('[WhatsApp] Erro ao abrir grupo:', e.message);
    return false;
  }
}

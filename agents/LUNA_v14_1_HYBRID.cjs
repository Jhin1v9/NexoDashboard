...
[CONFIG.REPORTS_DIR, CONFIG.ARTIFACTS_DIR, CONFIG.DEBUG_DIR].forEach(d => {
...
class LunaAgent {
  ...
  async init(options = {}) {
    ...
    this.client = new Client({
      authStrategy: new LocalAuth({ dataPath: path.join(CONFIG.ARTIFACTS_DIR, 'wwebjs-auth') }),
      puppeteer: {
        headless: false,
    ...
    const readyPromise = new Promise((resolve, reject) => {
      ...
      this.client.on('ready', async () => {
        try {
          ...
          if (once) await this.client.destroy();
          resolve(result);
        } catch (error) {
          reject(error);
        }

...
const SELECTORS = {
  ...
  // Sender: procura span com title (nome do contato) ou data-testid diferente de msg-meta
  sender: 'span[title]:not([data-testid="msg-meta"] *)',
  senderAlt: '[data-testid="msg-container"] div:first-child span',
    ...
    const batch = await page.evaluate((cfg) => {
      ...
      containers.forEach(container => {
        try {
          ...
          let sender = '';
          // Estratégia 1: span com title (nome do contato)
          ...
          // Estratégia 2: procura por elemento com title attribute
          if (!sender) {
          ...
          // Estratégia 3: procura em elementos irmãos antes do texto
          if (!sender) {
...
export async function runAgent() {
  ...
  try {
    ...
    // 8. Notifica Centro de Operações
    try {
      await fetch(`${CONFIG.DASHBOARD_API}/api/ops/changes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          type: 'whatsapp',
          message: `WhatsApp Agent: ${dashboardData.stats.totalMessages} mensagens extraídas, ${dashboardData.stats.totalTasks} tarefas, ${dashboardData.stats.totalIdeas} ideias`,
          source: 'whatsapp-agent',
          metadata: { reportId, stats: dashboardData.stats }
        })
      });
    } catch {}
